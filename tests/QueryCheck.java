...
class S2{
  ...
  public S2 (String _sname, String _bname) {
    sname = _sname;
    bname = _bname;
  }
}
...
public class QueryCheck {
  ...
  public QueryCheck(int q_index) {
    ...
    Q1result.addElement (new S1("Jarek Szlichta", "05/10/15"));
    Q1result.addElement (new S1("David Dewitt", "05/11/15"));
    Q1result.addElement (new S1("Parke Godfrey", "05/12/15"));
		
	Q2result.addElement (new S2("David Dewitt", "Enterprise"));
	Q2result.addElement (new S2("Parke Godfrey", "Voyager"));
	Q2result.addElement (new S2("Jarek Szlichta", "Buckey"));
	Q2result.addElement (new S2("Jarek Szlichta", "Wisconsin"));
	Q2result.addElement (new S2("Aijun An", "Buckey"));
	Q2result.addElement (new S2("Aijun An", "Wisconsin"));
	Q2result.addElement (new S2("Tom Reps", "Onion"));

    Q3result.addElement (new S3("Jarek Szlichta"));
    Q3result.addElement (new S3("Jarek Szlichta"));
    Q3result.addElement (new S3("Jarek Szlichta"));    
    Q3result.addElement (new S3("David Dewitt"));
    Q3result.addElement (new S3("David Dewitt"));
    Q3result.addElement (new S3("Parke Godfrey"));
    Q3result.addElement (new S3("Aijun An"));
    Q3result.addElement (new S3("Yannis Ioannidis"));
    Q3result.addElement (new S3("Raghu Ramakrishnan"));
    Q3result.addElement (new S3("Raghu Ramakrishnan"));

  
    Q4result.addElement (new S3("David Dewitt"));
    Q4result.addElement (new S3("Parke Godfrey"));
    Q4result.addElement (new S3("Jarek Szlichta"));
    Q4result.addElement (new S3("Aijun An"));
    Q4result.addElement (new S3("Raghu Ramakrishnan"));
    Q4result.addElement (new S3("Yannis Ioannidis"));

   
    Q5result.addElement (new S5("Jarek Szlichta", 9, (float)40.3));
    Q5result.addElement (new S5("Jarek Szlichta", 9, (float)40.3));
    Q5result.addElement (new S5("Jarek Szlichta", 9, (float)40.3));
    Q5result.addElement (new S5("David Dewitt",10, (float)53.2));
    Q5result.addElement (new S5("David Dewitt",10, (float)53.2));
    Q5result.addElement (new S5("Parke Godfrey", 5,(float) 32.3));
    Q5result.addElement (new S5("Yannis Ioannidis", 8, (float)46.2));

    Q6result.addElement (new S3("David Dewitt"));
    Q6result.addElement (new S3("Jarek Szlichta"));
    Q6result.addElement (new S3("Raghu Ramakrishnan"));
    Q6result.addElement (new S3("Yannis Ioannidis"));
    ...
    case 2:
      types[0] = new AttrType(AttrType.attrString);    // atrrtype array.
      types[1] = new AttrType(AttrType.attrString);
      sizes[0] = (short)20 ;                  // string length. 
      sizes[1] = (short)20;
      columnum = (short)2;                    // colum number.
      groupnum = 1;                    // group number.
      tuplenum = 7;                    // the total number of answer.
      grouporder = new Order(Order.SORT);
      mygroup[0].len = 7;              // each group length.
      mygroup[0].count = 0;            // count for correct answers
      mygroup[0].order = new Order(Order.UNSORT); // the tuple in group is sorted or not
      for(int i=0; i<mygroup[0].len;i++)  {// set tuple value. 
	try {
          ...
          mygroup[0].mytuple[i].setStrFld(2,((S2)Q2result.elementAt(i)).bname); 
	}
	catch (Exception e) {
	  System.err.println ("**** Error setting up the tuples");
	}
    ...
    case 3:
	  ...
	  try {
	    mygroup[0].mytuple[i] = new Tuple();
	    mygroup[0].mytuple[i].setHdr(columnum, types, sizes);
	    mygroup[0].mytuple[i].setStrFld(1,((S3)Q3result.elementAt(i)).sname);
	  }
	  catch (Exception e) {
	    System.err.println ("**** Error setting up the tuples");
	  }
    ...
    case 4:
	  ...
	  try {
	    mygroup[0].mytuple[i] = new Tuple();
	    mygroup[0].mytuple[i].setHdr(columnum, types, sizes);
	    mygroup[0].mytuple[i].setStrFld(1,((S3)Q4result.elementAt(i)).sname);
	  }
	  catch (Exception e) {
	    System.err.println ("**** Error setting up the tuples");
	  }
    ...
    case 6:
	  ...
	  try {
            mygroup[0].mytuple[i] = new Tuple();
            mygroup[0].mytuple[i].setHdr(columnum, types, sizes);
            mygroup[0].mytuple[i].setStrFld(1,((S3)Q6result.elementAt(i)).sname);
	  }
	  catch (Exception e) {
	    System.err.println ("**** Error setting up the tuples");
	  }
